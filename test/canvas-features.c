...
{
	GtkCanvasItem *parent1;
	GtkCanvasItem *parent2;
	...
	if (item->parent == parent1)
		gtk_canvas_item_reparent (item, GTK_CANVAS_GROUP (parent2));
	else
		gtk_canvas_item_reparent (item, GTK_CANVAS_GROUP (parent1));
...
create_canvas_features (void)
{
	...
	gtk_canvas_set_scroll_region (GTK_CANVAS (canvas), 0, 0, 400, 200);
	gtk_container_add (GTK_CONTAINER (frame), canvas);
	gtk_widget_show (canvas);

	/* First parent and box */

	parent1 = gtk_canvas_item_new (gtk_canvas_root (GTK_CANVAS (canvas)),
					 gtk_canvas_group_get_type (),
					 "x", 0.0,
					 "y", 0,0,
					 NULL);

	gtk_canvas_item_new (GTK_CANVAS_GROUP (parent1),
	...
	parent2 = gtk_canvas_item_new (gtk_canvas_root (GTK_CANVAS (canvas)),
					 gtk_canvas_group_get_type (),
					 "x", 200.0,
					 "y", 0,0,
					 NULL);

	gtk_canvas_item_new (GTK_CANVAS_GROUP (parent2),
			       gtk_canvas_rect_get_type (),
			       "x1", 0.0,
			       "y1", 0.0,
			       "x2", 200.0,
	...
	item = gtk_canvas_item_new (GTK_CANVAS_GROUP (parent1),
	...
	group = gtk_canvas_item_new (GTK_CANVAS_GROUP (parent2),
				       gtk_canvas_group_get_type (),
				       "x", 100.0,
				       "y", 100.0,
				       NULL);

	gtk_canvas_item_new (GTK_CANVAS_GROUP (group),
			       gtk_canvas_ellipse_get_type (),
			       "x1", -50.0,
			       "y1", -50.0,
			       "x2", 50.0,
