...
VarImpBase* GeBoolVar::clone(void) {
  ...
  Assert(gs);
  BoolView bv(static_cast<BoolVarImp*>(gs->getVar(index)));
  BoolVar *v = new BoolVar(bv);
  BoolVar x;
  x.update(gs,false,*v);
  delete v;
  return x.var();
...
bool GeBoolVar::intersect(TaggedRef x) {
  ViewRanges<BoolView> gvr(getBoolView());
  
  //Ask Alejandro about the use of getGSpace() instead of oz_currentBoard()
  return (get_BoolView(x).inter_r(oz_currentBoard()->getGenericSpace(),gvr)==ME_GEN_FAILED ? false: true);
}
...
bool GeBoolVar::In(TaggedRef lx) {
  return getBoolView().in(oz_intToC(lx));
}
...
bool GeBoolVar::hasSameDomain(TaggedRef v) {
  ...
  Assert(OZ_isGeBoolVar(v));
  ViewRanges< BoolView > vr1 (get_BoolView(v));
  ViewRanges< BoolView > vr2 (getBoolView());
...
bool GeBoolVar::IsEmptyInter(TaggedRef* var1,  TaggedRef* var2) {
  ...
  ViewRanges<BoolView > vr1 (get_BoolView(*var1));
  ViewRanges<BoolView > vr2 (get_BoolView(*var2));
