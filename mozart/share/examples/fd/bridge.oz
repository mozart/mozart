...
%%% FIXME: Why is it blocking only in GeOz?
...
bridge(tasks:
	  ...
	  proc {$ Start Dur}
		 ...
		 {FD.sumC [1 1 ~1 ~1] [Start.B Dur.B Start.A Dur.A] '=<:' 4}
	      end}
	     {ForAll [a1#s1 a2#s2 a5#s5 a6#s6 p1#s3 p2#s4]
	      proc {$ A#B}
		 %Start.B - (Start.A + Dur.A) =<: 3
		 {FD.sumC [1 ~1 ~1] [Start.B Start.A Dur.A] '=<:' 3}
	      end}
	     {ForAll [s1 s2 s3 s4 s5 s6]
	      proc {$ A}
		 %Start.A >=: Start.ue + 6
		 {FD.sumC [1 ~1] [Start.A Start.ue] '>=:' 6}
	      end}
	     {ForAll [m1 m2 m3 m4 m5 m6]
	      proc {$ A}
		 %(Start.A + Dur.A) - 2 =<: Start.ua
		 {FD.sumC [1 1 ~1] [Start.A Dur.A Start.ua] '=<:' 2}
	      end}
	     %Start.l =: Start.pa + 30
	     {FD.sumC [1 ~1] [Start.l Start.pa] '=:' 30}
	     Start.pa = 0
	  end)
   ...
   in
	 ...
	 {ForAll TaskSpec
	  proc {$ T}
		 ...
		 {FD.sumC [1 1 ~1] [Start.P Dur.P Start.{Label T}] '=<:' 0}
	      end}
	  end}
...
proc {Earlier Old New}
   %Old.pe >: New.pe
   {FD.greater Old.pe New.pe}
end

{Show {SearchBest {Compile Bridge} Earlier}}
