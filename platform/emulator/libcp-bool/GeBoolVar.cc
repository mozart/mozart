...
bool GeBoolVar::intersect(TaggedRef x) {
  BoolVar *gv = getBoolVarInfoPtr();
  ViewRanges<BoolView> gvr(*gv);
  delete gv;
  
  BoolVar *liv = get_BoolVarInfoPtr(x);
  BoolView vw(*liv);
  delete liv;
...
bool GeBoolVar::hasSameDomain(TaggedRef v) {
  ...
  Assert(OZ_isGeBoolVar(v));
  BoolVar *v1 = get_BoolVarInfoPtr(v);
  BoolVar *v2 = getBoolVarInfoPtr();
