...
VirtualSite* VSSegmentImportersRegister::find(VirtualSite *vs)
{
  ...
  while(aux) {
    VirtualSite *auxVS;
    auxVS = (VirtualSite*)(void*) aux->getBaseKey();

    //
    if (vs == auxVS) {
      VirtualSite *fvs;
      fvs = (VirtualSite*)(void*) aux->getEntry();
      return (fvs);
    }
...
void VSSegmentImportersRegister::insert(VirtualSite *vs)
{
  ...
  //
  ghn_bk = (GenHashBaseKey*)(void*) vs;
  ghn_e = (GenHashEntry*)(void*) vs;
  //
...
void VSSegmentImportersRegister::retract(VirtualSite *vs)
{
  ...
  while (ghn) {
    VirtualSite *auxVS;
    auxVS = (VirtualSite*)(void*) ghn->getBaseKey();
...
VirtualSite *VSSegmentImportersRegister::getFirst()
{
  ...
  if (seqGHN) {
    vs = (VirtualSite*)(void*) seqGHN->getEntry();
  } else {
    vs = (VirtualSite *) 0;
  }
  return (vs);
...
VSMsgChunkPoolSegmentManagerImported* VSChunkPoolRegister::find(key_t key)
{
  ...
  while(aux) {
    ...
    if (key == auxKey) {
      ...
      msgChunkPoolManager = (VSMsgChunkPoolSegmentManagerImported*)(void*) aux->getEntry();
...
void VSChunkPoolRegister::add(key_t key, VSMsgChunkPoolSegmentManagerImported *pool)
{
  ...
  //
  ghn_bk = (GenHashBaseKey*)(void*) key;
  ghn_e = (GenHashEntry*)(void*) pool;
  htAdd(hvalue, ghn_bk, ghn_e);
}
