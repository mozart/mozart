...
OZ_BI_define(BIsetAnnotation,2,0)
{
  OZ_declareTerm(0,entity);
  oz_declareIntIN(1,annot);
  if(oz_isVarOrRef(entity)){
    TaggedRef e = oz_deref(entity);
    if(oz_isVar(e)){
      oz_getNonOptVar((TaggedRef*)entity);
    }
    //printf("Converted var to simple %s\n", toC(entity));
  }
  annotateEntity(entity,annot);
...
void cellOperationDoneReadImpl(Tertiary*, TaggedRef, int);

OZ_BI_define(BIremoteExecDone,3,0){
  OZ_Term tert    =   OZ_in(0);
  OZ_Term ans     =   OZ_in(1);
  OZ_Term id      =   OZ_in(2); 
  
  Tertiary *ter = static_cast<Tertiary *>( tagged2Const(tert));
  int id_int = OZ_intToC(id); 
  cellOperationDoneReadImpl(ter, ans, id_int); 
  return PROCEED;
}OZ_BI_end
