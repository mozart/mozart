...
OZ_C_proc_begin(BIchooseSpace, 2) {
  ...
  if (isSmallInt(choice_tag)) {
    ...
    oz_typeError(1, "Int or pair of Ints");
  }
			   ...
			   Bool recordFlag)
{
  ...
  if (arity == makeTaggedNULL()) {
    oz_typeError(1,"list of pairs of Feature and Value)");
  }
  DEREF(t0,t0Ptr,tag0);
...
OZ_C_proc_begin(BIarrayNew,4)
{
  ...
  if (!isSmallInt(low)) {
    oz_typeError(0,"(small) Int");
  }
  if (!isSmallInt(high)) {
    oz_typeError(1,"(small) Int");
  }
...
OZ_C_proc_begin(BIdictionaryItems,2)
{
  ...
  return OZ_unify(tagged2Dictionary(dict)->items(),out);
}
OZ_C_proc_end
...
OZ_C_proc_begin(BIdictionaryClone,2)
{
  ...
  return OZ_unify(tagged2Dictionary(dict)->clone(),out);
}
OZ_C_proc_end
...
OZ_Return dictionaryGetInline(TaggedRef d, TaggedRef k, TaggedRef &out)
{
  ...
  if (dict->getArg(key,out) != PROCEED) {
    return am.raise(E_SYSTEM,E_KERNEL,"dict",2,d,k);
  }
  return PROCEED;
}
...
OZ_C_proc_begin(BImakeClass,9)
{
  ...
  if (!isAbstraction(send))      { oz_typeError(3,"Procedure (no builtin)"); }
