...
void printTime(FILE *fd,const char *s,unsigned int t)
{
  fprintf(fd,"%s",s);
  if (t < 1000) {
    fprintf(fd,"%u ms",t);
...
static
void printPercent(FILE *fd,const char *s,unsigned int t,unsigned int total)
{
  fprintf(fd,"%s",s);
  if (total == 0) {
    fprintf(fd,"0");
...
void printMem(FILE *fd,const char *s,double m)
{
  fprintf(fd,"%s",s);
  if (m < KB) {
    fprintf(fd,"%.0f B",m);
