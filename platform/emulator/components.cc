...
public:
  ...
  virtual char const *getHeader() = 0;
...
private:
  ...
  char *header;
public:
  char const *getHeader() {return header; }
...
class ByteSink {
public:
  OZ_Return putTerm(OZ_Term,char const*,char const*,unsigned int,Bool,Bool);
  virtual OZ_Return putBytes(BYTE*,int) = 0;
  virtual OZ_Return allocateBytes(int,char const*,unsigned int,crc_t,Bool) = 0;
};
...
public:
  ByteSinkFile(char const *s,int lvl): filename(s),fd(-1),zfd(0), compressionlevel(lvl) {}
  virtual ~ByteSinkFile() { 
    if (zfd!=0) { gzclose(zfd); return; }
    if (fd!=-1) close(fd);
  }
  OZ_Return allocateBytes(int,char const*,unsigned int,crc_t,Bool);
...
OZ_Term makeGenericExc(char const *id,char const *msg, OZ_Term arg)
{
...
OZ_Return
ByteSink::putTerm(OZ_Term in, char const *filename, char const *header,
		  unsigned int hlen,
...
OZ_Return
ByteSinkFile::allocateBytes(int n,char const* header, unsigned int hlen, crc_t crc, Bool txtmode)
{
...
OZ_Return
ByteSinkDatum::allocateBytes(int n, char const*, unsigned int, crc_t crc_ignored,
...
static
OZ_Return saveIt(OZ_Term val, char const *filename, char const *header,
		 unsigned int hlen,
...
static
void doRaise(TaggedRef controlvar, char const *msg, const char *url,URLAction act)
{
