...
Bool getDefaultPropertyBool(char const *p,Bool def)
{
  char *str=osgetenv(p);
  if (!str) return def;
  char *rest;
...
void ConfigData::init() {
  ...
  printFloatPrecision   = PRINT_FLOATPRECISION;
  printScientificFloats = PRINT_SCIENTIFICFLOATS;
