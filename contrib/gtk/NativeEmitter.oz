...
define
   local
      %% Hack: Something goes wrong with these symbols: Omit them (Windows specific)
      ErrorSymbols = [%% Missing GDK Functions
		      ...
		      "gtk_binding_entry_add_signall" "gtk_binding_entry_remove"
		      ...
		      "gtk_draw_extension" "gtk_draw_box_gap" "gtk_draw_shadow_gap"
		      "gtk_draw_tab" "gtk_draw_ramp" "gtk_draw_option" "gtk_draw_check"
		      "gtk_draw_flat_box" "gtk_theme_engine_unref"
		      "gtk_arg_to_valueloc" "gtk_arg_info_hash" "gtk_arg_info_equal"
		      ...
		      "gtk_identifier_get_type" "gtk_propagate_event" "gtk_window_reposition"
   ...
   in
	 ...
	 in
	    ...
	    IsDot = if IsVoid then false else Emitter, emitArgs(0 Args false $) end
	 ...
	 meth emitArgument(I Arg $)
	    ArgDecl#Dot = case Arg
			  of arg(type("..." "") _)          then {Base "GOZ_declareTerm"}#t
			  [] arg(type(Type "") _)           then {DeclareCoreType @types Type}#f
			  [] arg(type("char" "*") _)        then {Base "GOZ_declareString"}#f
	 ...
	 meth emitReturnType(RetType)
	    ...
	    [] type(Type "")   then TextFile, putS({Util.toString "\t"#Type#" ret;"})
	    [] type(Type Ptrs) then TextFile, putS({Util.toString "\t"#Type#" "#Ptrs#"ret;"})
	    end
	 end
	 ...
	 in
	    ...
	    in
	       ...
	       TextFile, putS({ToS "\t\t"#Ret#Key#"("#Args#", OZ_intToC("#DA#"), NULL);"})
	       TextFile, putS({ToS "\t} else if (OZ_isFloat("#DA#")) {"})
	       TextFile, putS({ToS "\t\t"#Ret#Key#"("#Args#", OZ_floatToC("#DA#"), NULL);"})
	       TextFile, putS({ToS "\t} else if (OZ_isVirtualString("#DA#", NULL)) {"})
	       TextFile, putS({ToS "\t\t"#Ret#Key#"("#Args#", GOZ_stringToC("#DA#"), NULL);"})
	       TextFile, putS({ToS "\t} else if (OZ_isForeignPointer("#DA#")) {"})
	       TextFile, putS({ToS "\t\t"#Ret#Key#"("#Args#
			       ", OZ_getForeignPointer("#DA#"), NULL);"})
	       TextFile, putS("\t} else {")
	       TextFile, putS({ToS "\t\t"#Ret#Key#"("#{ArgChain 0 (IA - 2)}#", NULL);"})
	       TextFile, putS("\t}")
	    else
	       Args = if IA == 0 then "" else {ArgChain 0 (IA - 1)} end
	    in
	 ...
	 in
	    TextFile, putS({Util.toString "\t{\""#Export#"\", "#IA#", "#OA#", native_"#Name#"},"})
	 end
