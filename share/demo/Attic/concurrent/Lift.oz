...
local
   ...
   fun {MakeLift IMPORT}
      ...
      = case Tk.isColor
	then [steelblue bisque lightsteelblue blue 'IndianRed']
	else [grey55 grey55 grey55 black black]
	end
      ...
      %%
      %% catch exceptions due to closed objects
      %%
...
%	 end
	 ...
	 meth proceed
	    ...
	    in
	       Lift, draw(case To>@CurrFloor then liftUp
			  elsecase To<@CurrFloor then liftDown
			  else lift end)
	       
	       case {Abs CurrPos-PosTo}<5
	       then
		  Lift, reached
		  
	       elsecase CurrPos>PosTo
	       then
	    ...
	    in
		  ...
		  {CatchClosed
	       ...
	       Agenda <- {Insert
			  goto(To
			       SendMode
			       case To>Num then up else down end
			       Who)
			  @CurrFloor
			  @Agenda}
	 ...
	 meth press
	    R={RanChoiceFloor}
	    Action = case R==NumFloors then doDown
		     elsecase R==1 then doUp
		     else {RanChoiceBin doUp doDown} end
	 in
