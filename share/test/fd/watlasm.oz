...
body
   ...
   fd([watlasm([
		...
		{MiscTest 1
		 fun {$}
		    if {FD.reflect.nextSmaller I 100} = 10
		    then 1 else {Show f#1} 0 end
		 end}
		
		{MiscTest 2
		 fun {$}
		    if {FD.reflect.nextSmaller I 10} = _
		    then {Show f#2} 0 else 1 end
     		 end}

		{MiscTest 3
		 fun {$}
		    if {FD.reflect.nextSmaller B 100} = 1
		    then 1 else {Show f#3} 0 end
		 end}

		{MiscTest 4
		 fun {$}
		    if {FD.reflect.nextSmaller B 1} = 0
		    then 1 else {Show f#4} 0 end
		 end}

		{MiscTest 5
		 fun {$}
		    if {FD.reflect.nextSmaller B 0} = _
		    then {Show f#5} 0 else  1 end
		 end}

		{MiscTest 6
		 fun {$}
		    if {FD.reflect.nextSmaller B ~10} = _
		    then {Show f#27} 0 else 1 end
		 end}
     
		{MiscTest 7
		 fun {$}
		    if {FD.reflect.nextSmaller FI 100} = 30
		    then 1 else {Show f#6} 0 end
		 end}
		    
		{MiscTest 8
		 fun {$}
		    if {FD.reflect.nextSmaller FI 30} = 29
		    then  1 else {Show f#7} 0 end
		 end}
		    
		{MiscTest 9
		 fun {$}
		    if {FD.reflect.nextSmaller FI 11} = 10
		    then  1 else {Show f#8} 0 end
		 end}

		{MiscTest 10
		 fun {$}
		    if {FD.reflect.nextSmaller FI 10} = _
		    then {Show f#9} 0 else  1 end
		 end}

		{MiscTest 11
		 fun {$}
		    if {FD.reflect.nextSmaller FI 1} = _
		    then {Show f#10} 0 else  1 end
     		 end}

		{MiscTest 12
		 fun {$}
		    if {FD.reflect.nextSmaller FB 100} = 30
		    then 1 else {Show f#11} 0 end
		 end}

		{MiscTest 13
		 fun {$}
		    if {FD.reflect.nextSmaller FB 31} = 30
		    then 1 else {Show f#12} 0 end
		 end}

		{MiscTest 14
		 fun {$}
		    if {FD.reflect.nextSmaller FB 30} = 29
		    then 1 else {Show f#13} 0 end
		 end}

		{MiscTest 15
		 fun {$}
		    if {FD.reflect.nextSmaller FB 21} = 20
		    then 1 else {Show f#14} 0 end
		 end}

		{MiscTest 16
		 fun {$}
		    if {FD.reflect.nextSmaller FB 20} = 6
		    then 1 else {Show f#15} 0 end
		 end}

		{MiscTest 17
		 fun {$}
		    if {FD.reflect.nextSmaller FB 10} = 6
		    then 1 else {Show f#16} 0 end
		 end}

		{MiscTest 18
		 fun {$}
		    if {FD.reflect.nextSmaller FB 4} = 2
		    then 1 else {Show f#17} 0 end
		 end}

		{MiscTest 19
		 fun {$}
		    if {FD.reflect.nextSmaller FB 2} = _
		    then {Show f#18} 0 else 1 end
		 end}

		{MiscTest 20
		 fun {$}
		    if {FD.reflect.nextSmaller FL 100000} = 30000
		...
		{MiscTest 21
		 fun {$}
		    if {FD.reflect.nextSmaller FL 30001} = 30000
		    then 1 else {Show f#20} 0 end
		 end}

		{MiscTest 22
		 fun {$}
		    if {FD.reflect.nextSmaller FL 30000} = 29999
		...
		{MiscTest 23 
		 fun {$}
		    if {FD.reflect.nextSmaller FL 20001} = 20000
		    then  1 else {Show f#22} 0 end
		 end}

		{MiscTest 24
		 fun {$}
		    if {FD.reflect.nextSmaller FL 20000} = 6000
		    then  1 else {Show f#23} 0 end
		 end}
		    
		{MiscTest 25
		 fun {$}
		    if {FD.reflect.nextSmaller FL 10000} = 6000
		    then  1 else {Show f#24} 0 end
		 end}

		{MiscTest 26
		 fun {$}
		    if {FD.reflect.nextSmaller FL 4000} = 2000
		    then  1 else {Show f#25} 0 end
		 end}

		{MiscTest 27
		 fun {$}
		    if {FD.reflect.nextSmaller FL 2000} = _
     ...
     %% nextLarger
		{MiscTest 28
		 fun {$}
		    if {FD.reflect.nextLarger I 1} = 10
		    then 1 else {Show f#101} 0 end
		 end}

		{MiscTest 29
		 fun {$}
		    if {FD.reflect.nextLarger I 10} = _
		    then {Show f#102} 0 else  1 end
		 end}
     
		{MiscTest 30
		 fun {$}
		    if {FD.reflect.nextLarger B ~100} = 0
		    then 1 else {Show f#103} 0 end
		 end}

		{MiscTest 31
		 fun {$}
		    if {FD.reflect.nextLarger B 0} = 1
		    then  1 else {Show f#104} 0 end
		 end}

		{MiscTest 32
		 fun {$}
		    if {FD.reflect.nextLarger B 1} = _
		    then {Show f#105} 0 else 1 end
		 end}

		{MiscTest 33
		 fun {$}
		    if {FD.reflect.nextLarger B 100} = _
		    then {Show f#1027} 0 else 1 end
		 end}
     
		{MiscTest 34
		 fun {$}
		    if {FD.reflect.nextLarger FI 1} = 10
		    then  1 else {Show f#106} 0 end
		 end}

		{MiscTest 35
		 fun {$}
		    if {FD.reflect.nextLarger FI 10} = 11
		    then  1 else {Show f#107} 0 end
		 end}

		{MiscTest 36
		 fun {$}
		    if {FD.reflect.nextLarger FI 29} = 30
		    then 1 else {Show f#108} 0 end
		 end}

		{MiscTest 37
		 fun {$}
		    if {FD.reflect.nextLarger FI 30} = _
		    then {Show f#109} 0 else 1 end
		 end}

		{MiscTest 38
		 fun {$} 
		    if {FD.reflect.nextLarger FI 100} = _
		    then {Show f#110} 0 else 1 end
		 end}
     
		{MiscTest 39
		 fun {$}
		    if {FD.reflect.nextLarger FB 1} = 2
		    then  1 else {Show f#111} 0 end
		 end}

		{MiscTest 40
		 fun {$}
		    if {FD.reflect.nextLarger FB 2} = 4
		    then  1 else {Show f#112} 0 end
		 end}

		{MiscTest 41
		 fun {$}
		    if {FD.reflect.nextLarger FB 10} = 20
		    then  1 else {Show f#113} 0 end
		 end}

		{MiscTest 42
		 fun {$}
		    if {FD.reflect.nextLarger FB 19} = 20
		    then 1 else {Show f#114} 0 end
		 end}

		{MiscTest 43 
		 fun {$}
		    if {FD.reflect.nextLarger FB 20} = 21
		    then 1 else {Show f#115} 0 end
		 end}

		{MiscTest 44
		 fun {$}
		    if {FD.reflect.nextLarger FB 29} = 30
		    then  1 else {Show f#116} 0 end
		 end}

		{MiscTest 45
		 fun {$}
		    if {FD.reflect.nextLarger FB 30} = _
		    then {Show f#117} 0 else  1 end
		 end}

		{MiscTest 46
		 fun {$}
		    if {FD.reflect.nextLarger FB 200} = _
		...
		{MiscTest 47
		 fun {$} F = {FD.int [10#40 45 50#100]}
		    ...
		    if FS = {FD.reflect.size F}
		    then skip else {Show f200b} R=0 end
		    F \=: 45
		    thread
		       if F = 40 then R=1 else {Show f200c} R=0 end
		    end
		    R
		 end}
		...
		{MiscTest 48
		 fun {$}
		    ...
		    if FMin = {FD.reflect.min F}
		    then skip else {Show f201b} R=0 end
		    F >: 10
		    thread
		       if F = 40 then  R=1 else {Show f201c} R=0 end
		    end
		    R
		 end}
		...
		{MiscTest 49
		 fun {$}
		    ...
		    if FMax = {FD.reflect.max F}
		    then skip else {Show f202b} R=0 end
		    F <: 100
		    thread
		       if F = 40 then  R=1 else {Show f202c} R=0 end
		    end
		    R
		 end}
