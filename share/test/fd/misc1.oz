...
in
   fd([misc1([
		 ...
		 R = thread if D = 0 then 1 else 0 end end
		 X = Y
		 R
	      end}
		 ...
		 L ::: 0#FD.sup
		 S :: 0#FD.sup
		 R = thread if {FD.sum L '=:' S} then 1 else 0 end end
	     ...
	     {MiscTest 4
	      fun {$}
		 if X in X::0#1 X::1#2 then 1 else 0 end
	      end}

	     {MiscTest 5
	      fun {$}
		 if X in X::0#1 X::2#3 then 0 else 1 end
	      end}
     
	     {MiscTest 6
	      fun {$}
		 if X in X::0#1 X::1 then 1 else 0 end
	      end}
     
	     {MiscTest 7
	      fun {$}
		 if X in X::0#10 X::20#25 then 0 else 1 end
	      end}
		 ...
		 X=1
		 Z=1
		 thread if Y :: [0 2#10] then 1 else 0 end end 
	      end}
     
	     {MiscTest 11
	      fun {$} L X Y Z R in  L=[X Y Z] L:::0#10
		 thread R = if {FD.atMost 2 r(a:X b:Y c:Z) 1}
		 ...
		 thread R = if Y = 1 then 1 else 0 end end
                    % V in 0#3	
		 X=1
                    % V in 0#3 
		 Z\=:1
	     ...
	     {MiscTest 15
	      fun {$} X R in X::1#4 
		 thread R = if {FD.element X 1#0#0#1 1} then 0 else 1 end end
		 X::2#3
		 R
	      end}
		 ...
		 {FD.sumAC [1 ~1] [X Y] '>=:' 8}
		 thread R = if Y::[9#10] then 1 else 0 end end
                    % X,Y in [0#2 8#10]
		 X=1
                    %Y in 9#10
		 R
	     ...
	     {MiscTest 17
	      fun {$} X Y R in [X Y] ::: 0#10
		 {FD.sumAC [1 ~1] [X Y] '=:' 8}
		 thread R = if 9 = Y then 1 else 0 end end
                    % X,Y in [0#2 8#10]
		 X=1
                    % Y=9
		 R
	     ...
	     {MiscTest 18
	      fun {$} X Y R in [X Y] ::: 0#10
		 {FD.sumAC [1 ~1] [X Y] '>:' 8}
		 thread R = if 10 = Y then 1 else 0 end end
                    % X,Y in [0#1 9#10]
		 X=1
                    %Y = 10
		 R
	     ...
	     {MiscTest 19
	      fun {$} X Y in [X Y] ::: 0#10
		 {FD.sumAC [1 1] [X Y] '=<:' 3}
		 thread if [X Y] ::: 0#3 then 1 else 0 end end
	      end}
	     
	     {MiscTest 20
	      fun {$} X Y in [X Y] ::: 0#10
		 {FD.sumAC [1 1] [X Y] '<:' 3}
		 thread if [X Y] ::: 0#2 then 1 else 0 end end
	      end}
		 ...
		 X<:2 Y>:8
		 thread if  B = 1 then 1 else 0 end end
	      end}
		 ...
		 B = {FD.reified.sumAC [1 ~1] [X Y] '>:' 5}
		 X>:7 Y>:7
		 thread if B = 0 then 1 else 0 end end
	      end}
	     
	     {MiscTest 24
	      fun {$} R X Y Z in [X Y Z]:::0#10
		 {FD.sumACN [1 ~1] [[X Y] [Z]] '>:' 8}
		 thread R = if X :: [0#1 9#10]
			       Z :: [0#1 9#10]
			    then 1 else 0 end
		 end
		 Y=1
		 ...
		 thread R = if Y :: 3#7 then 1 else 0 end end 
		 X=2
                    % Y in 3#7
		 R
	      end}
		 ...
		 thread if [X Y] ::: 2#10 then 1 else 0 end end
	      end}
	     
	     {MiscTest 31
	      fun {$} X Y in [X Y]:::0#10
		 {FD.sumCN 1#1 '#'([X] a(a:Y b:Y)) '>:' 25}
		 thread if X::0#10 Y::2#10 then 1 else 0 end end
	      end}
	     
	     {MiscTest 32
	      fun {$}  X Y Z in [X Y Z]:::0#10
		 {FD.sumCN 1#1 '#'([X] a(a:Y b:Z)) '>:' 25}
		 thread if X :: 0#10 [Y Z] ::: 2#10 then 1 else 0  end end
	      end}
		 ...
		 {FD.sumC 1#4 [X Y] '<:' 5}
                    % X in 0#4, Y in 0#1
		 thread if X ::0#4 Y::0#1 then 1 else 0 end end
	      end}
		 ...
		 {FD.distance X Y '>:' 8}
                    % X,Y in [0#1 9#10]}
		 thread if [X Y] ::: [0#1 9#10] then 1 else 0 end end
	      end}
		 ...
		 thread R = if Y :: 0#2 then 1 else 0 end end
		 condis X*X=:9 2*Y<:4
		 [] X+Y<:5
		 end
		 X=2
		 ...
		 thread R = if X::1#8 Y :: 1#8 then 1 else 0 end end
		 condis X*Y<:9 
		 [] X<:7 Y<:7
		 end
                    % X,Y in 1#8
	     ...
	     {MiscTest 40
	      fun {$} X Y R in [X Y]:::0#10
		 thread R = if {FD.distance X Y '\\=:' 3} then 0 else 1 end end
		 X=1 Y=4
		 R
	      end}
		 ...
		 X <: Y
		 thread if X = 3 Y = 4 then 1 else 0 end end 
	      end}
	     
	     {MiscTest 42
	      fun {$} Z = {FD.int 0#1} in
		 {FD.conj 0 Z Z}
		 thread if Z = 0 then 1 else 0 end end 
	      end}
	     
	     {MiscTest 43
	      fun {$} R L X Y Z in [X Y Z] = L L ::: 0#1
		 thread R = if L ::: 0 then 1 else 0 end end
		 Z = X+Y=:2
		 X=Z
	     ...
	     {MiscTest 44
	      fun {$} 
		 if X={FD.decl} in {FD.distinctOffset [X X] [100 100]}
		 then 0 else 1 end
	      end}
		 ...
		 R = thread if {Wait {SearchOne Test}} then 1 else 0 end end
		 X = 0
		 R
	      end}
	    ])
