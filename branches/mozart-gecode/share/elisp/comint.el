...
;;; I hope that this generalizes shell mode, lisp mode, tea mode, soar mode,...
...
;;;     comint-last-input-start - marker           ...
...
;   (kill-all-local-variables) ; Removed 1/15/90 Olin
    ...
    ;a buffer.
    (set-input-ring (if (ring-p old-ring)
                         old-ring
...
(defun make-comint (name program &optional startfile &rest switches)
  (let* ((buffer
	  ;; Don't create the buffer in any other mode, since
	  ;; we are going to set it to comint-mode.  This fixes
...
(fset 'make-shell 'make-comint) ; mostly compatible...
...
;;; use-buffer-local-input-ring to NIL.
(defvar use-buffer-local-input-ring nil)

;;; A list of vectors of the form [<buffer-name> <input-ring>]; these let
;;; the input history be associated with the name of the buffer, so that it
;;; persists from one instance of the buffer to another (especially useful
;;; with gdb buffers), and even one invocation of emacs to another.
(defvar *buffer-histories-list* nil)

(defun get-input-ring ()
  (if use-buffer-local-input-ring
      (and (boundp 'input-ring)
           input-ring)
      (let ((list *buffer-histories-list*)
            (buffer (current-buffer))
            (ring nil))
        (while (and list (not ring))
          (cond ((eq buffer (get-buffer (aref (car list) 0)))
                 (aset (car list) 1 buffer)
                 (setq ring (aref (car list) 2)))
                ((eq buffer (aref (car list) 1))
                 (aset (car list) 0 (buffer-name buffer))
                 (setq ring (aref (car list) 2))))
          (setq list (cdr list)))
        (if (not ring)
            (set-input-ring nil)
...
(defun set-input-ring (ring-to-use)
  (if (not (ring-p ring-to-use))
      (setq ring-to-use (make-ring input-ring-size)))
  (if use-buffer-local-input-ring
      (progn
        (make-local-variable 'input-ring)
        (setq input-ring ring-to-use))
      (progn
        (setq *buffer-histories-list*
              (cons (vector (buffer-name (current-buffer)) (current-buffer) ring-to-use)
                    *buffer-histories-list*))
        ring-to-use)))
...
(defun ring-head (ring)
  (car ring))
(defun set-ring-head (ring value)
  (let ((len (length (cdr (cdr ring)))))
       (if (or (< value 0)
               (>= value len))
           (error (format "Attempt to make head of ring (size = %d) be %d"
                          len value)))
       (setcar ring value)))

(defun ring-tail (ring)
  (car (cdr ring)))
(defun set-ring-tail (ring value)
  (let ((len (length (cdr (cdr ring)))))
       (if (or (< value 0)
               (>= value len))
           (error (format "Attempt to make head of ring (size = %d) be %d"
                          len value)))
       (setcar (cdr ring) value)))
...
(defun ring-plus (index increment veclen)
  "INDEX+INCREMENT, with wraparound."
  ;; use comint-mod here as we don't have control over increment
  ;; arg since this function is called with user-supplied increment
  (comint-mod (+ index increment) veclen))

(defun ring-length (ring)
  "Number of elts in the ring."
  (let ((hd (ring-head ring)) 
        (tl (ring-tail ring)) 
        (siz (length (ring-vector ring))))
...
item to make room."       
  (let* ((vec (ring-vector ring))  
         (len (length vec))
	 (new-hd (ring-minus1 (ring-head ring) len)))
    (set-ring-head ring new-hd)
    (aset vec new-hd item)
    (if (ring-empty-p ring)                 ;overflow -- dump one off the tail.
        (set-ring-tail ring (ring-minus1 (ring-tail ring) len)))))

(defun ring-remove-oldest (ring)
  "Remove the oldest item retained on the ring."
  (if (ring-empty-p ring) (error "Ring empty")
      (let ((tl (ring-tail ring))  
            (vec (ring-vector ring)))
	(set-ring-tail ring (ring-minus1 tl (length vec)))
	(aref vec tl))))

(defun ring-remove-newest (ring)
  "Remove the newest item retained on the ring."
  (if (ring-empty-p ring) (error "Ring empty")
      (let ((hd (ring-head ring))  
            (vec (ring-vector ring)))
	(set-ring-head ring (ring-plus1 hd (length vec)))
	(aref vec hd))))
...
(defun ring-search-and-destroy (ring victim predicate)
  "Search for a matching victim in the ring according to predicate
and destructively remove it if found."
  (cond ((not (ring-empty-p ring))
	 (let* ((hd (ring-head ring))
		(tl (ring-tail ring))
		(vec (ring-vector ring))
		(len (length vec))
		(top (if (>= tl hd) (+ tl 1) len))
		(i hd)
		(done nil))
	   (while (and (not done) (< i top))
	     (cond ((funcall predicate victim (aref vec i))
		    (while (> i hd)
		      (aset vec i (aref vec (- i 1)))
		      (setq i (- i 1)))
		    (set-ring-head ring (ring-plus1 hd len))
		    (setq done t)))
	     (setq i (+ i 1)))
	   (cond ((and (not done) (< tl hd))
		  (setq i 0)
		  (while (and (not done) (<= i tl))
		    (cond ((funcall predicate victim (aref vec i))
			   (while (< i tl)
			     (aset vec i (aref vec (+ i 1)))
			     (setq i (+ i 1)))
			   (set-ring-tail ring (ring-minus1 tl len))
			   (setq done t)))
		    (setq i (+ i 1)))))))))
    ...
    (cond ((<= len 0)
	  ...
	  (t
	   (cond ((eq last-command 'comint-previous-input)
		  (delete-region comint-last-input-start (point)))
	   ...
	   (insert (ring-ref ring input-ring-index))
...
; (interactive "sCommand substring: ")
  ...
  (let* ((ring (get-input-ring))
    ...
    (while (and (< n len) (not (string-match str (ring-ref ring n))))
      (setq n (+ n 1)))
    (cond ((< n len)
...
in the history, if -1 it will go forward."
  ...
  (let* ((size (length comint-last-similar-string))
         (ring (get-input-ring))
	 (len (ring-length ring))
	 (limit (if (= input-ring-index -1)
		    (if (= arg 1)
			0
			(- len 1))
		    (ring-plus input-ring-index arg len)))
	 (n limit)
	 (done nil)
	 entry)
    (while (and (not done)
		(or (< (length (setq entry (ring-ref ring n))) size)
		    (not (string-equal comint-last-similar-string 
				       (substring entry 0 size)))))
      (setq n (ring-plus n arg len))
      (if (= n limit)
	  (setq done t)))
    (cond ((not done)
	   (setq input-ring-index n)
	   (if (eq last-command 'comint-previous-similar-input)
	       (delete-region comint-last-input-start (point)) ; repeat
               (set-marker comint-last-input-start (point))) ; 1st time
...
in the history, if -1 it will go back."
  (interactive "p")
	...
	(let* ((ring (get-input-ring))
	       ...
	       (input (if (>= (point) pmark-val)
			  ...
			  (let ((copy (funcall comint-get-old-input)))
			    ...
			    copy))))
	  (insert ?\n)
	  (cond ((funcall comint-input-filter input)
		 (ring-search-and-destroy
		   ring input (function string-equal))
    ...
    (cond ((null completions)
	  ...
	  (t
	   (let ((conf (current-window-configuration)))
	     ...
	     (let ((ch (next-command-event (allocate-event))))
	       (if revert-soonest 
                   (progn
                     (if (key-press-event-p ch)
                         (set-window-configuration conf)
                         (setq not-reverted nil))
                     (dispatch-event ch))
