...
Gecode::ExecStatus DisjointProp::propagate(Gecode::Space *s) {
  ...
  if(xu + xd <= yl) return Gecode::ES_SUBSUMED;
  if(yu + yd <= xl) return Gecode::ES_SUBSUMED;
  ...
  if(lowx <= upx) {
    ...
    GECODE_ME_CHECK(x0.minus(s,la));
  }
  
  if(lowy <= upy) {
    Gecode::IntVar Tmp(s,lowy,upy);
    Gecode::Int::ViewRanges<IntView> la(Tmp);
    GECODE_ME_CHECK(x1.minus(s,la));
  }
