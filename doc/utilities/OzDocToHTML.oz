   ...
   local
      ...
      fun {RecordToTuple R}
         {List.toRecord {Label R}
          {Filter {Record.toListInd R} fun {$ X#_} {IsInt X} end}}
      end
      ...
      class OzDocToHTML
         ...
         in
            ...
            if Tag == SGML.namePI then
               ...
               [] span then
                  if {IsOfClass M ignore} then skip
                  else
                     OzDocToHTML, Batch(M 1)
                  end
               [] def then
               ...
               [] table then Title Mr in
                  %--** display attribute
                  case {GetSubtree {RecordToTuple M} title ?Mr} of unit then
                     skip
                  else
                     {Exception.raiseError ozDoc(sgmlToHTML unsupportedTableTitle M)}
                  end
         ...
         in
            ...
            if {IsOfClass M unnumbered} then
               if Title == unit then
         ...
         meth OutputFigure(M) Class N Number Title Mr1 Caption Mr2 in
            ...
            Title = {GetSubtree {RecordToTuple M} title ?Mr1}
            Caption = {GetSubtree Mr1 caption ?Mr2}
